...
export default function EditPages(){
  ...
  useEffect(()=>{ fetch('/api/admin/item?type=pages&slug=' + slug).then(r=>r.json()).then(j=>{ setData(j.data||{}); setContent(j.content||'') }) },[slug])
  async function save(){ setSaved(false); const res = await fetch('/api/admin/item?type=pages&slug=' + slug,{method:'POST',body:JSON.stringify({data,content})}); if(res.ok) setSaved(true) }
  async function del(){ if(!confirm('Eliminar?')) return; const res = await fetch('/api/admin/item?type=pages&slug=' + slug,{method:'DELETE'}); if(res.ok) window.location.href='/admin/pages' }
