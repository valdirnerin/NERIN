...
export default function PagesAdmin(){
  ...
  useEffect(()=>{ fetch('/api/admin/list?type=pages').then(r=>r.json()).then(setItems) },[])
  async function create(){
    const s = slug.trim().toLowerCase().replace(/\s+/g,'-'); if(!s) return
    const res = await fetch('/api/admin/item?type=pages&slug=' + s, { method:'POST', body: JSON.stringify({data:{title:s}, content:''}) })
    if(res.ok) window.location.href='/admin/pages/' + s
  }
  return (<div className="py-10">
    <h1 className="text-3xl font-extrabold mb-6">Pages</h1>
