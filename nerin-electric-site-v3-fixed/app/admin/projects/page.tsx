...
export default function ProjectsAdmin(){
  ...
  useEffect(()=>{ fetch('/api/admin/list?type=projects').then(r=>r.json()).then(setItems) },[])
  async function create(){
    const s = slug.trim().toLowerCase().replace(/\s+/g,'-'); if(!s) return
    const res = await fetch('/api/admin/item?type=projects&slug=' + s, { method:'POST', body: JSON.stringify({data:{title:s}, content:''}) })
    if(res.ok) window.location.href='/admin/projects/' + s
  }
